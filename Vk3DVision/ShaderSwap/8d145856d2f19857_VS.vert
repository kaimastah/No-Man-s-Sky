...
{
    mat4 _605 = _640;
    _605[0] = mkTransformMat4R0;
    mat4 _607 = _605;
    _607[1] = mkTransformMat4R1;
    mat4 _609 = _607;
    _609[2] = mkTransformMat4R2;
    mat4 _611 = _609;
    _611[3] = mkTransformMat4R3;
    ...
    vec4 _474 = _229.lUniforms.mpCommonPerMesh.gWorldMat4 * mkLocalPositionVec4;
    vec4 _477 = _620 * _474;
    mat4 _518 = mat4(vec4(normalize(vec3(mkTransformMat4R0.xyz)), 0.0), vec4(normalize(vec3(mkTransformMat4R1.xyz)), 0.0), vec4(normalize(vec3(mkTransformMat4R2.xyz)), 0.0), vec4(0.0, 0.0, 0.0, 1.0));
    vec3 _248 = normalize((_518 * vec4((_229.lUniforms.mpCommonPerMesh.gWorldNormalMat4 * vec4(normalize(mkLocalNormalVec4.xyz), 1.0)).xyz, 1.0)).xyz);
    Out.mWorldPositionVec3_mfSpare = vec4(_477.x, _477.y, _477.z, Out.mWorldPositionVec3_mfSpare.w);
    Out.mWorldPositionVec3_mfSpare = vec4(_477.x, _477.y, _477.z, Out.mWorldPositionVec3_mfSpare.w);
    vec3 _591 = (_518 * vec4((_229.lUniforms.mpCommonPerMesh.gWorldNormalMat4 * vec4(normalize(mkTangentVec4.xyz), 1.0)).xyz, 1.0)).xyz;
    vec3 _290 = normalize(_591);
    Out.mTangentMatRow1Vec3 = _290;
    vec3 _295 = normalize(cross(_248, _591) * ((mkTangentVec4.w > 0.00999999977648258209228515625) ? 1.0 : (-1.0)));
    ...
    Out.mTangentSpaceEyeVec3 = _336 * normalize(_477.xyz - _229.lUniforms.mpPerFrame.gViewPositionVec3);
...
Out.mTangentSpaceEyeVec3.xyz += vk3d_params.stereo.x * vk3d_params.stereo.y * Out.mTangentSpaceLightDirVec3.xyz * _229.lUniforms.mpPerFrame.gViewProjectionMat4[0].xyz / fov;
    ...
    vec4 _599 = _229.lUniforms.mpPerFrame.gViewProjectionMat4 * _477;
    Out.mScreenSpacePositionVec4 = _599;

// Vk3DVision (http://3dsurroundgaming.com/) - Stereo Injection
Out.mScreenSpacePositionVec4.x += vk3d_params.stereo.x * (Out.mScreenSpacePositionVec4.w - vk3d_params.stereo.y); 
    Out.mPrevScreenPosition = _229.lUniforms.mpPerFrame.gPrevViewProjectionMat4 * _477;
