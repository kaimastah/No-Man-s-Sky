...
{
    mat4 _604 = _639;
    _604[0] = mkTransformMat4R0;
    mat4 _606 = _604;
    _606[1] = mkTransformMat4R1;
    mat4 _608 = _606;
    _608[2] = mkTransformMat4R2;
    mat4 _610 = _608;
    _610[3] = mkTransformMat4R3;
    ...
    vec4 _473 = _229.lUniforms.mpCommonPerMesh.gWorldMat4 * mkLocalPositionVec4;
    vec4 _476 = _619 * _473;
    mat4 _517 = mat4(vec4(normalize(vec3(mkTransformMat4R0.xyz)), 0.0), vec4(normalize(vec3(mkTransformMat4R1.xyz)), 0.0), vec4(normalize(vec3(mkTransformMat4R2.xyz)), 0.0), vec4(0.0, 0.0, 0.0, 1.0));
    vec3 _248 = normalize((_517 * vec4((_229.lUniforms.mpCommonPerMesh.gWorldNormalMat4 * vec4(normalize(mkLocalNormalVec4.xyz), 1.0)).xyz, 1.0)).xyz);
    Out.mWorldPositionVec3_mfSpare = vec4(_476.x, _476.y, _476.z, Out.mWorldPositionVec3_mfSpare.w);
    Out.mWorldPositionVec3_mfSpare = vec4(_476.x, _476.y, _476.z, Out.mWorldPositionVec3_mfSpare.w);
    vec3 _590 = (_517 * vec4((_229.lUniforms.mpCommonPerMesh.gWorldNormalMat4 * vec4(normalize(mkTangentVec4.xyz), 1.0)).xyz, 1.0)).xyz;
    vec3 _290 = normalize(_590);
    Out.mTangentMatRow1Vec3 = _290;
    vec3 _295 = normalize(cross(_248, _590) * ((mkTangentVec4.w > 0.00999999977648258209228515625) ? 1.0 : (-1.0)));
    ...
    Out.mTangentSpaceEyeVec3 = _336 * normalize(_476.xyz - _229.lUniforms.mpPerFrame.gViewPositionVec3);
...
Out.mTangentSpaceEyeVec3.xyz += vk3d_params.stereo.x * vk3d_params.stereo.y * Out.mTangentSpaceLightDirVec3.xyz * _229.lUniforms.mpPerFrame.gViewProjectionMat4[0].xyz / fov;
    ...
    vec4 _598 = _229.lUniforms.mpPerFrame.gViewProjectionMat4 * _476;
    Out.mScreenSpacePositionVec4 = _598;

// Vk3DVision (http://3dsurroundgaming.com/) - Stereo Injection
Out.mScreenSpacePositionVec4.x += vk3d_params.stereo.x * (Out.mScreenSpacePositionVec4.w - vk3d_params.stereo.y); 
    Out.mPrevScreenPosition = _229.lUniforms.mpPerFrame.gPrevViewProjectionMat4 * _476;
