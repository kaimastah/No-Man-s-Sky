...
void main()
{
    ...
    vec4 _376 = _112.lUniforms.mpCommonPerMesh.gWorldMat4 * mkLocalPositionVec4;
    vec3 _152 = normalize((_112.lUniforms.mpCommonPerMesh.gWorldNormalMat4 * vec4(normalize(mkLocalNormalVec4.xyz), 1.0)).xyz);
    Out.mWorldPositionVec3_mfSpare = vec4(_376.x, _376.y, _376.z, Out.mWorldPositionVec3_mfSpare.w);
    Out.mWorldPositionVec3_mfSpare = vec4(_376.x, _376.y, _376.z, Out.mWorldPositionVec3_mfSpare.w);
    vec3 _394 = (_112.lUniforms.mpCommonPerMesh.gWorldNormalMat4 * vec4(normalize(mkTangentVec4.xyz), 1.0)).xyz;
    vec3 _192 = normalize(_394);
    Out.mTangentMatRow1Vec3 = _192;
    vec3 _197 = normalize(cross(_152, _394) * ((mkTangentVec4.w > 0.00999999977648258209228515625) ? 1.0 : (-1.0)));
    ...
    Out.mTangentSpaceEyeVec3 = _238 * normalize(_376.xyz - _112.lUniforms.mpPerFrame.gViewPositionVec3);
...
Out.mTangentSpaceEyeVec3.xyz += vk3d_params.stereo.x * vk3d_params.stereo.y * Out.mTangentSpaceLightDirVec3.xyz * _112.lUniforms.mpPerFrame.gViewProjectionMat4[0].xyz / fov;
    ...
    vec4 _402 = _112.lUniforms.mpPerFrame.gViewProjectionMat4 * _376;
    Out.mfFadeValueForInstance_mfLodIndex_mfShearMotionLength.z = 0.0;
    vec4 _408 = _402;
    _408.z = 0.5 - (max(_402.z, -1.0) * 0.5);
    vec4 _410 = _408;
    _410.w = 1.0;
    gl_Position = _410;
