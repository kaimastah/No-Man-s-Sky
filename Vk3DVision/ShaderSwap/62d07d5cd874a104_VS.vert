...
void main()
{
    ...
    vec3 _159 = (_139.xyz + (_86.lUniforms.mpPerFrame.gCameraNoHeadTrackingMat4[0].xyz * (_113.x * mkCustom1Vec4.x))).xyz + (_86.lUniforms.mpPerFrame.gCameraNoHeadTrackingMat4[1].xyz * (_113.y * mkCustom1Vec4.x));
    float _346 = clamp((length(_159.xyz - _86.lUniforms.mpPerFrame.gCameraNoHeadTrackingMat4[3].xyz) - _86.lUniforms.mpCommonPerMesh.gLightColourVec4.x) * _86.lUniforms.mpCommonPerMesh.gLightColourVec4.y, 0.0, 1.0);
...
gl_Position.x += vk3d_params.stereo.x * gl_Position.w; 
    float _203 = 1.0 - _346;
    vec4 _354 = _363;
    _354.x = smoothstep(0.87999999523162841796875, 1.0, _203);
    vec4 _356 = _354;
    _356.y = smoothstep(0.959999978542327880859375, 1.0, _203);
    vec4 _358 = _356;
    _358.z = smoothstep(0.100000001490116119384765625, 1.0, _203);
    vec4 _360 = _358;
